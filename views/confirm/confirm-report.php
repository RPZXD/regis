...
<div class="space-y-6 animate-fade-in-up">
    ...
    <?php if (empty($studentData)): ?>
        ...
        <div class="max-w-md mx-auto glass rounded-2xl p-8 shadow-xl">
            <form method="POST" action="" class="space-y-6">
                ...
                <div>
                    <label
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">เลขประจำตัวประชาชน</label>
                ...
                <button type="submit" name="search"
                    class="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-bold hover:shadow-lg hover:scale-[1.02] transition-all">
                    ค้นหาข้อมูล
                </button>
            </form>
            <?php if (isset($error)): ?>
    ...
    <?php else: ?>
        ...
        <?php elseif ($currentStatus == 0): ?>
            ...
            <div class="max-w-2xl mx-auto glass rounded-2xl p-8 shadow-xl text-center">
                ...
                <div class="p-4 bg-amber-50 rounded-xl border border-amber-200 mb-6">
                    <p class="text-amber-700"><i class="fas fa-info-circle mr-2"></i>ข้อมูลของท่านอยู่ระหว่างการตรวจสอบ
                        กรุณารอผลการประกาศ</p>
                </div>
                <a href="index.php"
                    class="inline-block px-8 py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-xl font-bold hover:shadow-lg transition-all">
                    <i class="fas fa-home mr-2"></i>กลับหน้าหลัก
                </a>
            </div>
        ...
        <?php else: ?>
            ...
            <?php if (intval($studentData['is_called']) === 0): ?>
                <!-- Reservist State / Not Called Yet -->
                <div class="max-w-2xl mx-auto glass rounded-2xl p-8 shadow-xl text-center">
                    ...
                    <h2 class="text-2xl font-bold text-blue-700 mb-4">บัญชีสำรอง / รอเรียกตัว</h2>
                    <p class="text-gray-600 dark:text-gray-400 mb-2">
                        <strong><?php echo $studentData['stu_prefix'] . $studentData['stu_name'] . ' ' . $studentData['stu_lastname']; ?></strong>
                    </p>
                    <p class="text-gray-500 text-sm mb-6">เลขบัตร: <?php echo $studentData['citizenid']; ?></p>
                    <div class="p-4 bg-blue-50 rounded-xl border border-blue-200 mb-6">
                        <p class="text-blue-700"><i class="fas fa-info-circle mr-2"></i>ท่านผ่านการคัดเลือกและอยู่ในบัญชีรอเรียกตัว
                            กรุณารอดำเนินการหรือประกาศจากทางโรงเรียนก่อนจึงจะสามารถรายงานตัวได้</p>
                    </div>
                    <a href="index.php"
                        class="inline-block px-8 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-bold hover:shadow-lg transition-all">
                        <i class="fas fa-home mr-2"></i>กลับหน้าหลัก
                    </a>
                </div>

            <?php else: ?>
                <!-- Display Confirmation Information for Called Students -->
                <div class="max-w-4xl mx-auto glass rounded-2xl p-8 shadow-xl shadow-blue-500/10 relative overflow-hidden">
                    <div
                        class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-500/20 to-indigo-500/20 rounded-bl-full -mr-8 -mt-8">
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-8 items-start">
                        ...
                        <div class="flex-grow text-center md:text-left space-y-2">
                            ...
                            <?php if ($plan): ?>
                                <div
                                    class="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800">
                                    ...
                                    <p class="text-emerald-600 dark:text-emerald-300">
                                        <?php echo $plan['type_name'] . ' - ' . $plan['name']; ?>
                                    </p>
                                </div>
                            <?php else: ?>
                                <div
                                    class="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-800">
                ...
                <?php if ($plan): ?>
                    ...
                    <div class="max-w-4xl mx-auto space-y-6">
                        <!-- Maintenance Fees -->
                        <?php
                        $maintenance = array_filter($fees, function ($f) {
                            return $f['category'] == 'maintenance';
                        });
                        $mTotal1 = 0;
                        $mTotal2 = 0;
                        foreach ($maintenance as $f) {
                            $mTotal1 += $f['term1_amount'];
                            $mTotal2 += $f['term2_amount'];
                        }
                        ?>
                        <div class="glass rounded-2xl p-6">
                            <h3 class="text-lg font-bold text-gray-800 mb-4 border-l-4 border-emerald-500 pl-3">1. เงินบำรุงการศึกษา
                            </h3>
                                        ...
                                        <?php foreach ($maintenance as $fee): ?>
                                            <tr>
                                                ...
                                                <td class="px-4 py-3 text-right font-medium">
                                                    <?php echo number_format($fee['term1_amount'] + $fee['term2_amount'], 2); ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                        ...
                        <!-- Support Fees -->
                        <?php
                        $support = array_filter($fees, function ($f) {
                            return $f['category'] == 'support';
                        });
                        $sTotal1 = 0;
                        $sTotal2 = 0;
                        foreach ($support as $f) {
                            $sTotal1 += $f['term1_amount'];
                            $sTotal2 += $f['term2_amount'];
                        }
                        ?>
                        <div class="glass rounded-2xl p-6">
                        ...
                        <!-- Action Buttons -->
                        <?php if (isset($isReportPeriodValid) && !$isReportPeriodValid): ?>
                            <div class="text-center mt-8 p-6 bg-red-50 border border-red-200 rounded-xl">
                                <i class="fas fa-clock text-red-500 text-4xl mb-4"></i>
                                <h3 class="text-xl font-bold text-red-700 mb-2">ไม่อยู่ในช่วงเวลารายงานตัว</h3>
                                <p class="text-red-600 text-lg"><?php echo $reportScheduleMsg; ?></p>
                                <a href="index.php"
                                    class="inline-block mt-4 px-6 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors">กลับหน้าหลัก</a>
                            </div>
                        <?php else: ?>
                            <div class="flex flex-col md:flex-row gap-4 justify-center pt-8">
                                ...
                                <button onclick="confirmReport()"
                                    class="px-12 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold shadow-lg shadow-emerald-500/30 hover:scale-105 transition-all">
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            Swal.fire('เกิดข้อผิดพลาด', response.message || response.error || 'กรุณาลองใหม่อีกครั้ง', 'error');
                        }
                    },
                    error: function () {
